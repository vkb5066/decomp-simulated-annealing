...
echo "dir,workDir,nrg,nConv,irredKpts,aV,bV,cV,alpha,beta,gamma,vol" >$origLoc/output.csv
...
#Get the volume of the supercell (a dot b cross c)
...
set cz=`cat file__ | sed -n '5p' | awk '{print $3}'`	
...
echo "`pwd`,$wd,$nrg,$nConv,$ikp,$xV,$yV,$zV,$al,$be,$ga,$vol" >>$origLoc/output.csv
