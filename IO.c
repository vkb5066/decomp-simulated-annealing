...
ushort ReadParamFile(const char* fileName, double* rCut,
	...
	for(uint rlCount = 0; (readRad + readAnn + readCon) < 3u; ++rlCount){
		...
		if(strstr(line, "BEGIN ANNEAL")){
			fgets(line, LINESIZE, infile);
			*nSteps = (uint)strtoul(line, &next, BASE); 
			*startStep = (uint)strtoul(next, NULL, BASE);
			fgets(line, LINESIZE, infile);
			*startNrg = strtod(line, &next);
			*endNrg = strtod(next, NULL);
			fgets(line, LINESIZE, infile);
			*startCool = strtod(line, &next);
			*endCool = strtod(next, NULL);
			fgets(line, LINESIZE, infile);
			*alpha = strtod(line, NULL);
			fgets(line, LINESIZE, infile);
...
void UnknownEnvHandler(const ushort nSites, const site** sites,
	...
	printf("ERR: at step %u, the following config had %u missing envs:\n", 
		   step, nUnknowns);
