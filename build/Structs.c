...
//Sparse tree functions--------------------------------------------------------
#if 0
void Alloc_gb(gbranch** b, uint ind, uint cnt){
	*b = malloc(sizeof(gbranch));
	(*b)->thisArr = NULL;
	(*b)->envIndex = ind;
	(*b)->envCount = cnt;
...
void Dealloc_gb(gbranch** b){
	for(ushort i = 0; i < (*b)->nBranches; ++i){
		Dealloc_gb(&((*b)->branches[i]));
	}

	free(*b); ///do NOT free the sparse arr pointers - done elsewhere
}

//Adds a pointer to a sparse env decomp array to a tree.  Increments 
//(NOT SETS!) a success counter if a new env was added.  Returns a pointer to 
//either (a) the arr that was sucessfully added or (b) the arr that conflicts 
//with the arr that was attempted to be added
//toAdd is meant to be a sparse vector filled with nSitesTot values in total
uint* Add_gb_s(gbranch*restrict*restrict head, const uint*restrict toAdd,
			   const ushort nSitesTot, uint*restrict success){
	gbranch* itr = *head;
	ushort accum = 0;
	for(ushort itrS = 0; accum < nSitesTot; itrS += (ushort)2){
		///check to see if we have a path to walk down
		for(ushort j = 0; j < itr->nBranches; ++j){
			if(toAdd[itrS] == itr->branches[j]->envIndex &&
			   toAdd[itrS + (ushort)1] == itr->branches[j]->envCount){
				itr = itr->branches[j];
				goto Walked;
			}
		}
		...
		itr->nBranches++;
		itr->branches=realloc(itr->branches, itr->nBranches*sizeof(gbranch*));
		gbranch* thisBranch;
		Alloc_gb(&thisBranch, toAdd[itrS], toAdd[itrS + (ushort)1]);
		itr->branches[itr->nBranches - (ushort)1] = thisBranch;
		itr = thisBranch;

		Walked: NOP
		accum += toAdd[itrS + (ushort)1];
	}

	//Now, itr is at the end of the tree
	///Return the env pointer if an env exists here already
	if (itr->thisArr) return itr->thisArr;

	///If we're here, an arr does not exist
	itr->thisArr = toAdd;
	(*success)++;
	return itr->thisArr;
}
...
//Adds a pointer to a sparse env decomp array to a tree.  Increments 
//(NOT SETS!) a success counter if a new arr was added.  Returns a pointer to 
//either (a) the arr that was sucessfully added or (b) the arr that conflicts 
//with the arr that was attempted to be added
//toAdd is meant to be a dense vector of length len, filled with nEnvsTot
//environments in total
uint* Add_gb_d(gbranch*restrict*restrict head, const uint*restrict toAdd,
			   const ushort nSitesTot, const uint len, uint*restrict success){
	gbranch* itr = *head;
	for(uint i = 0; i < len; ++i){
		if(toAdd[i]){ ////zero values are not represented in the sparse arr
			///check to see if we have a path to walk down
			for(ushort j = 0; j < itr->nBranches; ++j){
				if(i == itr->branches[j]->envIndex &&
				   toAdd[i] == itr->branches[j]->envCount){
					itr = itr->branches[j];
					goto Walked;
				}
			}
			...
			////otherwise, make the path and then go down it
			itr->nBranches++;
			itr->branches = realloc(itr->branches, 
									itr->nBranches*sizeof(gbranch*));
			gbranch* thisBranch;
			Alloc_gb(&thisBranch, i, toAdd[i]);
			itr->branches[itr->nBranches - (ushort)1] = thisBranch;
			itr = thisBranch;
	...
	//Now, itr is at the end of the tree
	///Return the env pointer if an arr exists here already
	if(itr->thisArr) return itr->thisArr;

	///If we're here, an arr does not exist
	///add it, but make it a sparse representation
	uint* sparse = malloc(2*nSitesTot*sizeof(uint));
	DecompConv(nSitesTot, toAdd, &sparse);
	itr->thisArr = sparse;
	(*success)++;
	return itr->thisArr;
}

//Searches for an arr in the arr tree.  Returns a pointer to the arr 
//that was sucessfully found, or NULL if it was not found
//searchKey is expected to be a sparse uint array (the env decomps)
uint* Search_gb_s(const gbranch*restrict head, const uint*restrict searchKey,
				  const ushort nSitesTot){
	gbranch* itr = head;
	ushort accum = 0;
	for(ushort itrS = 0; accum < nSitesTot; itrS += (ushort)2){
		///check to see if we have a path to walk down
		for(ushort j = 0; j < itr->nBranches; ++j){
			///if so, go down it
			if(searchKey[itrS] == itr->branches[j]->envIndex &&
			   searchKey[itrS + (ushort)1] == itr->branches[j]->envCount){
				itr = itr->branches[j];
				goto Walked;
			}
		}
...
//Searches for an arr in the arr tree.  Returns a pointer to the arr 
//that was sucessfully found, or NULL if it was not found
//searchKey is expected to be a dense uint array (the env decomps)
uint* Search_gb_d(const gbranch*restrict head, const uint*restrict searchKey,
				  const uint len){
	gbranch* itr = head;
	for(uint i = 0; i < len; ++i){
		if(searchKey[i]){ ////zero values are not represented in the sparse arr
			///check to see if we have a path to go down
			for(ushort j = 0; j < itr->nBranches; ++j){
				///if so, go down it
				if(i == itr->branches[j]->envIndex && 
				   searchKey[i] == itr->branches[j]->envCount){
					itr = itr->branches[j];
					goto Walked;
				}
			}
